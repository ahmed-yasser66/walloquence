...
import LatestInfiniteScroll from "./latest-client";
...
export const metadata:Metadata = {
  title: "Latest Wallpapers - Community Uploads",
  description:
    "Discover the latest wallpapers shared by our community. Explore a diverse collection of high-quality images for your desktop background.",
}
        ...
        <Wrapper>
          <div className="flex flex-col justify-center gap-3 pt-7">
            <p className="flex items-center gap-3 text-3xl font-bold">
              <Clock size={45} color="#ad3" />
              <span className="dark:text-shadow-[1px_2px_4px_black]">
                Latest
              </span>
            </p>
            <p className="text-base font-semibold">
              New community wallpapers, just dropped.
            </p>
          </div>
        </Wrapper>
      </header>
...
async function RenderResult() {
  const initialResponse = await api.get.latest();
    ...
    <section>
      <Wrapper>
        <div className="mt-5 pt-5">
          ...
          <LatestInfiniteScroll enabled={initialData.length > 0} />
        </div>
      </Wrapper>
    </section>
  );
